				...
				if ($result1->rowCount() > 0) {
					while($Zone = $result1->fetchObject()) {
						...
						WHERE `Essais`.`Tournoi` = ".$Tournoi->Id."
							...
							AND (`Zones` >= ".$Zone->Id." OR `Essais`.`Réussite` IS NULL)";
						if ($Voie->Nb_Essais_Evalués != null) {
							$query .= " AND (SELECT COUNT(*) FROM `Essais` AS `E` WHERE `E`.`Tournoi` = ".$Tournoi->Id." AND `E`.`Voie` = ".$Voie->Id." AND `Evalué` IS NOT NULL AND 'Réussite' IS NULL AND `E`.`Utilisateur` = `Essais`.`Utilisateur` AND `E`.`Date` < `Essais`.`Date`) < ".$Voie->Nb_Essais_Evalués;
						}
						$query .= " GROUP BY `Essais`.`Utilisateur`";
						$result2 = $dbh->query($query);
						if ($result2->rowCount() > 0) {
